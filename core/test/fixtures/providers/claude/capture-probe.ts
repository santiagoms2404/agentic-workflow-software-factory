...
// the argv `ClaudeCodeAdapter.buildSpec` produces, and this file is how anyone
// can see that for themselves without spending a second live call.
//
// It lives under `core/test/fixtures/` and not under `core/src/` for the same
// reason `stub-provider.mjs` does: it has to spawn, and `node:child_process` is
// importable nowhere in `core/src` except `execution/transport-broker.ts`. A
// capture script that needed an exemption from that fence would weaken the
// invariant it exists to help test.
//
...
const PROBE_CWD = "/tmp/awsf-claude-probe";

const PROMPT =
  "Use the Glob tool to list the files in the current directory, " +
  "then reply with only the number of files you found.";

const out = process.argv[2];
if (out === undefined) throw new Error("usage: capture-probe.ts <out.jsonl>");

const spec = new ClaudeCodeAdapter().buildSpec({
  model: "claude:sonnet",
  prompt: PROMPT,
  cwd: PROBE_CWD,
