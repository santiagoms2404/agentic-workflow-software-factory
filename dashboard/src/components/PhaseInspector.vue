...
import { formatCost, formatUsage } from "../display.ts";
defineProps<{ detail: PhaseDetailResponse }>();
function json(value: unknown) { return JSON.stringify(value, null, 2); }
</script>
<template>
  <section class="phase-inspector" aria-label="Phase details">
    <section><h3>Agent config</h3><dl><template v-for="agent in detail.agents" :key="agent.agent"><dt>{{ agent.agent }}</dt><dd>{{ agent.requestedModel }} · {{ agent.resolvedModel ?? "unrecorded" }}</dd></template></dl></section>
    ...
    <section><h3>Gates ({{ detail.gates.length }})</h3><ul class="inspector-list"><li v-for="gate in detail.gates" :key="gate.id"><strong>{{ gate.passed ? "PASS" : "FAIL" }} {{ gate.gateId }}</strong><span>round {{ gate.round }} · {{ gate.kind }}</span><pre v-if="gate.checks !== null">{{ json(gate.checks) }}</pre></li></ul><p v-if="!detail.gates.length" class="empty-note">No gate results recorded.</p></section>
    <section><h3>Usage</h3><p class="usage-line">{{ formatUsage(detail.usage) }} tokens · {{ formatCost(detail.usage.costAuthority, detail.usage.estimatedCostUsd) }} · {{ detail.usage.usageAuthority }}</p></section>
