...
<template><section class="event-log" aria-label="Event log"><header><h3>Events ({{ events.length }})</h3><button v-if="hasMore" type="button" :disabled="loading" @click="load">{{ loading ? "Loading…" : "Load more events" }}</button></header><p v-if="error" role="alert" class="empty-note">{{ error }}</p><ol><li v-for="event in events" :key="event.id" :class="`event-${event.type}`"><time>{{ new Date(event.startedAt).toLocaleTimeString() }}</time><strong>{{ event.type }}</strong><span>{{ command(event) }}</span><em>{{ formatDuration(event.startedAt, event.endedAt) }}</em></li></ol><p v-if="!events.length && !loading" class="empty-note">No events recorded.</p></section></template>
