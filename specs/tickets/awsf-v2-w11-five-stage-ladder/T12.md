...
## Induced-timer observation

The induced `setTimeout(() => {}, 0)` in `core/src/stages/contract.ts` made
`no-stage-advance.test.ts` fail with this exact assertion diff:
...
+ [
+   'core/src/stages/contract.ts:90:1 uses setTimeout'
+ ]
- []
```

After removing the throwaway line, the fence passed 3/3. Final validation:
`npm run test:unit` passed 1436/1436 tests in 286.96 seconds;
