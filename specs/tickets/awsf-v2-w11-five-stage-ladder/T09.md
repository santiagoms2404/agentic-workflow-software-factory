...
## Induced-drift observation

Renaming only the first contract record from `init` to `initialize` made
`node --experimental-strip-types --test core/test/unit/stages/contract.test.ts` exit 1. The exact failure text was:
...
After restoring `init`, the same command passed: 2/2, exit 0.
