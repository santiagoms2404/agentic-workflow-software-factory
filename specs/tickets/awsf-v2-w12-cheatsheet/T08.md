...
This is an implementation task: the index and offline fences, which are the obligations the single-file decision created. It flips THIS plan's markers only; it never touches `specs/awsf-v2-plan.html`. Its serialization after T07 is deliberate because both tasks edit `core/test/unit/meta/cheatsheet-reconciliation.test.ts`, not because either fence depends on the other's logic.
