...
**C1. Reuse T01's helpers, do not restate them.** `core/src/contracts/shift-selection-record.ts` exports
`sealShiftManifest({ plan, milestones, tickets })`, which computes `manifestDigest`, and
`core/src/persistence/plan-ticket-body.ts` exports `readPlanTicketFile(path)` → `{ digest, body }` and
`ticketFileDigest(bytes)`. The per-ticket `digest` is sha256 hex over the whole file.

**C2. `path` in the manifest is repository-relative; `PlanTicketRecord.path` is absolute.** The
schema refuses an absolute path, a `..` segment and a backslash. Whoever builds manifest entries
from `PlanTicketReader` records must relativise the path against the repository root first.

**C3. The schema is a backstop, not your refusal vocabulary.** `milestones` has `minItems: 1` and
`uniqueItems: true`, and `tickets` has `minItems: 1`, so a manifest with a repeated milestone or
no tickets already fails validation. The named error classes this prompt asks for are still owed:
they fire before a manifest exists and say which rule was broken.

**C4. `workflow: shift` is already unrepresentable in a manifest.** The ticket entry's `workflow`
reuses `stringUnion(TICKET_WORKFLOWS)`. Your meta-test over the ticket corpus and the vocabulary is
still owed; it guards the source, the schema only guards the record.

**C5. Markers: the fence overrules the README on milestone headers.** The ticket/plan-sync fence
...
The fence (code) wins: leave M1 at `[wip]`; T03 flips it to `[x]`.

**C6. RENAMED: no tracked file may carry `manifest` in its name.** AGENTS.md invariant 10's
...
Name no new file `*manifest*` either.
