---
id: T11
title: "A failing ticket blocks the attempt, and the prefix resumes"
milestone: M4
state: todo
depends_on: [T07]
serves: [AC-4]
---
# T11 · A failing ticket blocks the attempt, and the prefix resumes

> **Deep plan:** [`../../awsf-v2-w17-shift.html`](../../awsf-v2-w17-shift.html) § Implementation Phases → milestone M4 → task 11  
> **Spine:** [`../../awsf-v2-plan.html`](../../awsf-v2-plan.html) § Implementation Phases → Milestone M17 → W17  
> **Prompt source:** [`../../awsf-v2-w17-shift-build-prompts.md`](../../awsf-v2-w17-shift-build-prompts.md) § Section B — carried verbatim below.  
> **Serves:** AC-4 (declared in the plan's Identifier Spine)

## What this ticket is

An **implementation** task. It flips THIS plan's own markers in `specs/awsf-v2-w17-shift.html` and this ticket's `state:`. It does **not** touch the spine.

## Read first

The shared **Conventions** — the read-first set, marker discipline, the never-do list, the
commit rule and the baseline rule — are stated once in [`./README.md`](./README.md) §
Conventions and mirrored in [`../../awsf-v2-w17-shift-build-prompts.md`](../../awsf-v2-w17-shift-build-prompts.md) § Section A.
They apply to this ticket unchanged and are not repeated below.

## Handoff

_Empty at authoring time. Predecessor sessions append dated, numbered entries (C1, C2, …) here._
_It sits above the prompt rather than below it because the sync fence compares everything after_
_the prompt heading against Section B byte for byte — see [`./README.md`](./README.md) §_
_"Why the Handoff section comes first"._

### 2026-09-25 — from T04, via the plan's 2026-09-25 amendment

**C1. SCOPE added to this task: two runner gaps a shift hits first here.** The plan's task 11 now has
two new rows. (a) `production-run.ts`'s `engineer` branch must emit a shift brief's compiled `intent`
(`ShiftBriefPhase.intent` from `core/src/workflow/shift/compile.ts`) instead of `requestOutput(status)`.
Today every `t<NN>-brief` would carry the attempt's request. (b) `shift-review-context` must compose
every `t<NN>-brief`'s intent. Today review intent is `schemas.get("awsf.plan-output/v1")`, and last wins.
The plan wins over this ticket's prompt, which predates the finding.

**C2. Do not "fix" `{previous_envelope}`.** Ticket n's builder is rendered its own brief. Ticket n-1's
build envelope is only in the schema map. The plan's 2026-09-25 amendment records this as intended.

### 2026-09-25 — from T07, carrying forward T05's C3

**C3. FINDING: no task in this plan binds a sealed selection to an attempt.** `awsf start` on a shift
throws `CompiledWorkflowUnbound` (`core/src/workflow/catalog.ts`) because `AttemptStatus` has no field
for a sealed manifest — T05 built the refusal, not the binding, and named T11/T12 as the first tickets
that would need it. This task is the first to run a shift through `production-run.ts` (per its own
GATE G17-M and READ FIRST block), so it is the one that must: add the manifest-binding field to
`AttemptStatus`, and replace `CompiledWorkflowUnbound` with a real call to `compiledWorkflow("shift").compile`
once a manifest is bound. If T12 reaches this gap first instead, this finding applies there unchanged —
whichever of T11/T12 starts first owns the binding, and the other inherits it done.

### 2026-09-25 — from T09 (T11 does not depend on T09; this is here because T09 absorbed T11 scope)

**C1. SCOPE you can skip: the binding is built, and T10's C3 above is discharged.** The owner chose to pull
it into T09. `AttemptStatus.shift` holds the sealed manifest, set only by `newCommand({ shift })`, which
refuses a shift without one. `core/src/workflow/shift/bind.ts` (`bindShiftRecipe`) compiles it from the
canonical repository's ticket files. `awsf start`, the first run, the recovery binding and every resume entry
in `production-run.ts` resolve through `attemptRecipe`. No CLI flag passes a manifest to `awsf new` yet, so
journeys call `newCommand` directly. `shift-ceiling.test.ts` has a reusable `world()` you can copy.

**C2. Your task's `engineer`-branch row is built. Flip it with evidence, not new work.** The branch emits
`ShiftBriefPhase.intent`, and the T09 journey's builder asserts each prompt carries its own `TASK Tnn.` line.
The review-context row is NOT built: `shift-review-context` still judges against the last brief only.

**C3. Two runner gaps T09 fixed that your red-gate case depends on.** `shiftPhaseRole` maps each shift phase
to the shipped role it repeats, so ownership and the builder's write and diff gates apply to `t<NN>-build`.
Shift phases also measure `diff_matches_claims` from the head the phase started on, not the attempt base.
A red `t<NN>-tests` is measured against the `tests` role.

**C4. The ticket id is already on every checkpoint.** `PhaseRecovery.ticket` names the ticket of the next
unaccepted phase, and `shiftTicketOf(recipe.phases, phaseId)` in `bind.ts` resolves any phase to its ticket.
Use it for the blocker reason rather than parsing a phase id.

## Build prompt

```
[CHOOSE YOUR PROVIDER — pick by live quota]
  MODEL   Opus 5 · EFFORT high
  CLAUDE  claude:opus · /effort high
  GPT     codex:gpt-5.6-sol · reasoning high
  WHY     the decision is simple and its proof is not: nothing later may reserve a call, and
          everything earlier must survive

TASK 11 of 18. Plan: specs/awsf-v2-w17-shift.html, milestone M4.
PREDECESSORS: T07 are [x]

READ FIRST
  AGENTS.md - the twelve invariants, IN FULL
  specs/awsf-v2-w17-shift.html - the plan. Purpose, Problem, Solution and Identifier Spine IN FULL,
    then this task's own section
  specs/tickets/awsf-v2-w17-shift/README.md - Conventions, gates, never-do list
  GATE G17-M: main and task3.5 MUST be merged before this task starts. Confirm it and
    STOP if they are not - this milestone lives inside production-run.ts
  core/src/cli/commands/production-run.ts - acceptPhase, checkpointAt, and the BLOCKED path
  core/src/state/errors.ts - the six DETERMINISTIC_REASON_SOURCES and the per-edge
    blocker vocabulary
  core/src/cli/commands/rework.ts - IN FULL, to see why it is NOT the recovery instrument

DO
  Make a red t<NN>-tests phase block the attempt with a reason naming the TICKET ID and
    the failing gate, and record the accepted prefix in the recovery checkpoint.
  Prove no later ticket's phase reserves a call after the block, against the ledger's
    history() rather than against a log line.
  Make awsf resume re-run only the blocked ticket's phases, with the earlier tickets'
    commits still on the worktree head.
  Make an environmental failure distinguishable from a ticket failure in the record: the
    blocker carries one of the six deterministic reason sources, and a `gate` source must
    read differently from a `process` one.
  Add a meta-test asserting no shift module imports rework.ts, and write down why:
    every rework phase is maxCorrections 0, and that path REJECTS credential-shaped
    provider output where a normal run scrubs it.

DO NOT
  Do NOT make the shift skip a failed ticket and continue. Ticket n+1 was selected
    BECAUSE it follows ticket n; building it on a tree the gates rejected produces work
    nobody can review independently.
  Do NOT reach for rework as an automatic recovery. A blocked shift surfaces.

DONE WHEN
  npx tsx --test core/test/journeys/shift-blocked.test.ts - a six-ticket shift with a red
    gate on ticket 3 blocks there, prefix durable, tail unreserved, resume completes
  npm run test:unit, npm run typecheck, npm run lint - green, counts recorded

COMMIT
  When every DONE WHEN row is green, commit this task's work as ONE Conventional Commit:
    <type>(<scope>): <imperative summary, lower case, no trailing period, <=72 chars>

    <body: what changed and WHY - the decision behind the shape, the alternative
    rejected, and any row left [f] with the block that holds it. Wrap at 72 columns.>
  type in feat | fix | refactor | test | docs | chore | perf | build | ci
  scope = this plan's own vocabulary for the area (shift, compiler, budget, ref,
    readout, adopt) - never a file path.
  Include the plan HTML marker flips and this ticket's `state:` change IN THIS SAME COMMIT.
  Never put an agent, model, or AI tool in the commit identity, message, or trailers.
  If a DONE WHEN row cannot be made green, mark it [f], name the blocking condition in
  the body, and commit what is green - do not leave the tree dirty for the next session.

MARKERS
  Flip THIS leaf plan's own markers in specs/awsf-v2-w17-shift.html: task 11's checklist boxes
  from [] to [x] (or [f] with the reason in the commit body). The leaf markers are
  flipped on EVERY task, including this one.
  Flip this ticket's `state:` to done (or failed) in the same commit.
  Do NOT touch specs/awsf-v2-plan.html's Milestone M17 / W17 marker - only T18 does that.

HANDOFF
  Before you finish, open the ticket file(s) whose `depends_on` names this id and append
  to their `## Handoff` section anything that would otherwise make them start blind:
    - CONTRADICTIONS between their build prompt and what is now true (say which wins, and why)
    - MOVED or RENAMED files their READ FIRST block names
    - FINDINGS from this task that change what they should do
    - OPTIONS you considered and rejected, so they are not re-litigated
    - SCOPE they can now skip because this task already did it, or must now absorb
  Write it as dated, numbered entries (C1, C2, ...), each one actionable. Say plainly which
  source wins when two disagree - the plan, the code, or the prompt. Do NOT rewrite their
  build prompt: the prompt is the authored record, the Handoff is the correction layer.
  Append nothing if there is nothing - an empty Handoff is a real answer.
```
