...
- [ ] **The real T1 production command seam is zero-quota verifiable**: `build` uses one registered call, `plan-build-test` uses two separately registered calls, dashboard projection sees `RUNNING` before completion, host request/test envelopes and exact candidate gates reach `AWAITING_OWNER`, and unsupported/unavailable/registration/schema/permission/gate/command/projector negatives fail closed with no fallback or held reservation. — `journeys: core/test/journeys/production-runner.test.ts`.
