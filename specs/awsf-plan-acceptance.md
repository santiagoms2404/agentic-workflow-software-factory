...
- [x] **A journal written before a schema field existed still rebuilds.** An attempt journalled before the owner re-entry counter carries no `ownerReentries`, and `AttemptStatus` types it as a number — so a record handed straight to the projector is a value lying about its own type, `node:sqlite` refuses to bind it, and the rebuild dies on the first pre-upgrade attempt it meets. The legacy-budget upgrade is applied at every boundary where untyped JSON becomes an `AttemptStatus` — `readAttempt`, `deriveAttemptStatus`, and the rebuild's per-record replay — and never downstream of them, so the projection mapper still trusts its parameter type. Replays to `owner_reentries = 0`, which is the ledger's own answer, never null. — `unit: core/test/unit/cli/operator.test.ts`, verified to fail without the fix with the identical production error.
...
- [x] **The driving-document tree exists, and the four fences scoped to it now sweep a non-empty file list rather than reporting green over nothing.** `walkFiles` returns `[]` for a missing directory, so doc-reconciliation, no-handwritten-schema, no-`shell:true` and the credential sweep were all vacuous the day they landed; each proves its *matcher* bites, and none could prove the *walk* arrives. This row claims only that — the tree is scanned — and nothing about what any document says. — `unit: core/test/unit/meta/driving-tree.test.ts`, which fails if `docs/driving/` is moved, renamed or emptied.
- [x] **The router's route table cannot outlive the documents it names.** Every relative markdown path advertised in `docs/driving/skills/awsf/SKILL.md` resolves to a real file inside the tree, and the router advertises at least two — a table is edited when a document is *planned* and written later or not at all, so a route pointing at nothing is the drift a router is most prone to. A bare basename is deliberately not treated as a route: it names a file elsewhere in the repository, and asserting it resolves inside the tree would assert the opposite of the truth. — `unit: core/test/unit/meta/driving-routes.test.ts`.
...
- [x] **A tier-2 recipe reaches a provider through the real command path.** `awsf run` accepts `build-review`/`simple-sdlc` on a tier-2 attempt, and no persisted transition carries a hardcoded tier. `awsf rework` refuses T2 as `ReworkTierUnsupported` because it re-runs one builder phase and its candidate would carry no review. — `journeys: core/test/journeys/t2-production.test.ts`.
- [x] **The review provider is chosen by inversion, not by configuration.** The reviewer phase resolves through `oppositeProvider` over the providers the adapters *report*; an adapter configuration that disagrees raises `InvalidReviewInversion` before a call is spent; unavailability after one transport retry blocks on L17 and no substitute is attempted. — `journeys: core/test/journeys/t2-production.test.ts`.
- [x] **The inversion is provable after the fact.** `sessions.review_provider` and `sessions.review_verdict` are written by the production path, not merely read by the dashboard, and a review call never overwrites `worker_provider`. — `journeys: core/test/journeys/t2-production.test.ts` asserting both columns on one session row.
- [x] **The T2 landing guards receive evidence, never constants.** `requiredReviewPresent` is true only because L15 carried a completed opposite-provider review, and `journeyApproved` is false until the owner attests at a TTY through `awsf journey`, whose `journey_passes` report checks separately that the journey ran, passed, and named the exact candidate SHA. — `journeys: core/test/journeys/t2-production.test.ts`.
- [x] **An unevidenced review can be replaced once, at one call, without rebuilding the candidate.** `awsf review TASK --reason "..."` takes L25 at a TTY; eligibility is host-determined from the recorded `review_evidence_present` row, so a review carrying a passing one is refused as `ReviewNotReplaceable` at zero cost; the candidate is revalidated before authorization, immediately before `GO`, and again before L15; the replacement runs cold on the opposite provider under generation-qualified `reviewer-re<N>` identities that leave the superseded artefacts byte-identical; the tier ceiling must have two calls of headroom; and a crash before `GO` releases the reservation and rewinds the owner re-entry it charged. — `journeys: core/test/journeys/replacement-review.test.ts`.
- [x] **A replacement review that broke only the envelope contract spends its held retry instead of forfeiting the candidate.** The headroom rule reserves a second call for a retry, and it was reachable by transport faults alone — so a cold reviewer whose answer carried one redundant key per finding blocked the attempt on L17 with that call still unspent. A `continuity: "none"` route holds no state, so its recovery is a cold re-ask rather than a resume, and the second turn is handed the failing schema paths and nothing about what to conclude. Exhausting the retry rethrows the malformed failure as itself rather than as `MandatoryReviewUnavailable`, because the review was available and answered twice. Both turns are retained under distinct envelope rounds, which the projection's `INSERT OR REPLACE` on gate rows and `INSERT OR IGNORE` on envelopes would otherwise have collapsed silently. — `journeys: core/test/journeys/replacement-review.test.ts`, `unit: core/test/unit/workflow/review-retry.test.ts`.
- [x] **Pilot 2 landed a real T2 task with ≤ 5 calls; the review provider is provably the inverse of the worker provider in the session record; the end-user journey ran against the exact candidate SHA.** Landed at **2 of 5 calls** — one builder, one review — leaving three calls and the owner re-entry allowance unspent. `worker_provider` is `openai-codex` and `review_provider` is `anthropic` in the session row, so the inversion is read off the record rather than asserted. The journey was attested against the exact candidate SHA behind the owner-only TTY gate, and `journey_passes` carries its three separate checks. The review returned `concern` with four findings and **`review_evidence_present` passed** — the reviewer received the candidate's diff, so the verdict was earned rather than structurally guaranteed; the attempt was landed with those findings on the record. — `records/pilots/pilot-2.md` + `sessions.review_provider` vs `worker_provider` + `journey_passes` gate row.
- [x] **Nothing was deleted and nothing was pushed during either pilot.** Across both pilots no file, branch, ref, worktree, journal or attempt record was deleted; the superseded tier-2 attempt is retained unmodified including its review artefacts, and `awsf gc` lists cleanup candidates without removing any. Neither repository's remote was contacted — `main` sits ahead of its upstream locally in both — and no push, force or auto-delete path exists in `core/src` for either pilot to have used. — journal sweep + `git reflog` of the target repo.
