...
3. **The `awsf.config.yaml` amendment has landed as its own owner-authored commit**, before task 3.
   No agent writes that file; `path-policy` rejects `protected-path` independently of the write
   globs, and that is the boundary working. See the plan's M2 amendment card for the wording,
   the evidence, the replacement guarantee and the alternative that avoids it entirely.
...
GATE: the awsf.config.yaml amendment adding the guard's committed path to
policy.protected_paths must already have landed as its own owner-authored commit. If it
has not, stop. Do NOT write awsf.config.yaml - path-policy rejects protected-path
